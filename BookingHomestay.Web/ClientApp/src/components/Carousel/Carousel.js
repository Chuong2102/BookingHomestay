...
export default function Carousel({ children: slides, autoSlide = false, autoSlideInterval = 3000 }) {
    ...
    return (
        <div className={cx('overflow-hidden', 'relative', 'rounded-xl', 'aspect-square', 'w-full', 'h-full')}>
            <div
                className={cx('flex', 'transition-transform', 'ease-out', 'duration-500', 'w-full', 'h-full')}
            ...
            <div className={cx('absolute', 'bottom-4', 'right-0', 'left-0')}>
                <div className={cx('flex', 'items-center', 'justify-center', 'gap-2')}>
                    {slides.map((_, i) => (
                        <div
                            key={i}
                            className={cx('transition-all', 'w-2', 'h-2', 'mx-[1px]', 'bg-white', 'rounded-full', {
                                'p-1': curr === i,
